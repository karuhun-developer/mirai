...
<template>
  <div class="rounded-md border border-border bg-surface/40 p-3">
    <p class="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Kategori</p>

    <p v-if="all.length === 0" class="text-sm text-muted-foreground">
      Belum ada kategori. Buat dulu lewat tombol filter di halaman Library.
    </p>
    ...
    <div class="mt-3 flex justify-end gap-2">
      <Button variant="ghost" size="sm" @click="emit('close')">Tutup</Button>
      <Button v-if="all.length > 0" size="sm" @click="emit('save', checked)">Simpan</Button>
    </div>
  </div>
</template>
